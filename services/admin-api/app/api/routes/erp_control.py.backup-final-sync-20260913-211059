...
async def list_pharmacies(
    ...
    rows = (
        await db.execute(
            select(Pharmacy, OnboardingApplication.pharmacy_id)
            .join(OnboardingApplication, OnboardingApplication.user_id == Pharmacy.user_id)
            .where(OnboardingApplication.status == "approved")
...
async def erp_catalog_sync(
    ...
    """
    Machine-to-machine ERP synchronization.

    The ERP authenticates using the provisioned license envelope.
    Only an approved pharmacy with a valid, non-expired license can
    retrieve its ERP configuration catalog.
    """
    ...
    application = await get_application_for_pharmacy(pharmacy_id, db)

    if application.status != "approved":
        raise HTTPException(
            status_code=403,
            detail="ERP synchronization requires an approved pharmacy",
        )

    if not application.license_key:
        raise HTTPException(
            status_code=409,
            detail="ERP license key is not provisioned",
        )

    if application.license_expires_at:
        expires = application.license_expires_at
        ...
        if expires <= datetime.now(timezone.utc):
            raise HTTPException(
                status_code=403,
                detail="MEDORAX ERP license has expired",
            )

    if not compare_digest(
        str(application.license_key),
        str(x_erp_license_key),
    ):
        raise HTTPException(
            status_code=401,
    ...
    if not compare_digest(
        str(stored_signature),
        str(x_erp_license_signature),
    ):
        raise HTTPException(
            status_code=401,
    ...
    return {
        ...
        "licenseExpiresAt": application.license_expires_at,
        "options": options,
        "items": options,
        "count": len(options),
    }
