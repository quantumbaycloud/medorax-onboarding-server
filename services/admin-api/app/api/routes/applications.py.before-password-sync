...
async def approve_application(
    ...
    # ---------------------------------------------------------
    # Secure ERP credentials
    # ---------------------------------------------------------

    credential_result = await db.execute(
        select(ERPCredential).where(
            ERPCredential.application_id == row.id
        )
    )
    ...
    if credential is None:
        ...
        credential = ERPCredential(
            ...
            temporary_password_enc=encrypt(
                temporary_password
            ),
        ...
        username = credential.erp_username

        try:
            temporary_password = decrypt(
                credential.temporary_password_enc
            )
        except Exception:
            logger.exception(
                "Unable to decrypt existing ERP password "
                "for application %s",
                row.id,
            )
    ...
    # ---------------------------------------------------------
    # Build ERP provisioning payload
    #
    # temporaryPassword exists only in memory here.
    # It is NOT stored in erp_payload_json.
    # ---------------------------------------------------------
    ...
    # IMPORTANT:
    # Never persist the plaintext ERP password.
    safe_payload = dict(payload)
    safe_payload.pop("temporaryPassword", None)

    row.erp_payload_json = safe_payload
    ...
    provisioning_status = result.get(
        "status",
        "pending",
    )
    ...
    provisioning_error = result.get(
        "error"
    )

    row.erp_provision_status = provisioning_status
    row.erp_external_id = external_id
    row.erp_provision_error = provisioning_error

    credential.provision_status = provisioning_status
    credential.erp_external_id = external_id
