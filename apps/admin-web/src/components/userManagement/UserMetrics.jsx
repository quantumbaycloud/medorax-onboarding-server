          ...
          <div key={index} className="bg-[#ffffff] rounded-md p-4 shadow-sm flex items-center justify-between">
            <div className="flex flex-col">
              ...
              <span className={`text-[12px] font-medium flex items-center gap-1 mt-1 leading-4 ${
                ...
                {metric.change.includes('Review') ? '⚠' : <TrendingUp size={14} />}
                {metric.change}
              </span>
            </div>
            <div className={`w-10 h-10 rounded flex items-center justify-center ${metric.iconBg}`}>
              <Icon size={22} className={`text-${metric.color}`} />
            </div>
          </div>
        );
      })}
